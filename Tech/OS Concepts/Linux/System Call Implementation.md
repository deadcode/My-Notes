...
[Video of GDB doing breakpoint for a "dup2" syscall in a VM](https://linux-kernel-labs.github.io/refs/heads/master/lectures/syscalls.html#:~:text=it%20pushes%20userspace-,regs,-to%20stack)
...
| **System Call** | **Category**      | **Description**                                           | **Arguments**                                                        |
| --------------- | ----------------- | --------------------------------------------------------- | -------------------------------------------------------------------- |
| open            | File System       | Opens a file and returns a file descriptor.               | const char *pathname, int flags, mode_t mode                         |
| close           | File System       | Closes an open file descriptor.                           | int fd                                                               |
| read            | File System       | Reads data from a file descriptor into a buffer.          | int fd, void *buf, size_t count                                      |
| write           | File System       | Writes data from a buffer to a file descriptor.           | int fd, const void *buf, size_t count                                |
| lseek           | File System       | Moves the file offset for reading/writing.                | int fd, off_t offset, int whence                                     |
| stat            | File System       | Retrieves file metadata.                                  | const char *pathname, struct stat *statbuf                           |
| mkdir           | File System       | Creates a new directory.                                  | const char *pathname, mode_t mode                                    |
| rmdir           | File System       | Removes an empty directory.                               | const char *pathname                                                 |
| unlink          | File System       | Deletes a file.                                           | const char *pathname                                                 |
| rename          | File System       | Renames a file or directory.                              | const char *oldpath, const char *newpath                             |
| chdir           | File System       | Changes the current working directory.                    | const char *path                                                     |
| getcwd          | File System       | Retrieves the current working directory.                  | char *buf, size_t size                                               |
| chmod           | File System       | Changes file permissions.                                 | const char *pathname, mode_t mode                                    |
| chown           | File System       | Changes file ownership.                                   | const char *pathname, uid_t owner, gid_t group                       |
| fork            | Process Control   | Creates a new process by duplicating the current process. | void                                                                 |
| execve          | Process Control   | Executes a new program, replacing the current process.    | const char *filename, char *const argv[], char *const envp[]         |
| wait            | Process Control   | Waits for a child process to terminate.                   | int *wstatus                                                         |
| exit            | Process Control   | Terminates the calling process.                           | int status                                                           |
| kill            | Process Control   | Sends a signal to a process.                              | pid_t pid, int sig                                                   |
| getpid          | Process Control   | Returns the process ID of the calling process.            | void                                                                 |
| getppid         | Process Control   | Returns the parent process ID.                            | void                                                                 |
| brk             | Memory Management | Changes the location of the program break (heap end).     | void *end_data_segment                                               |
| mmap            | Memory Management | Maps a file or device into memory.                        | void *addr, size_t length, int prot, int flags, int fd, off_t offset |
| munmap          | Memory Management | Unmaps a memory region.                                   | void *addr, size_t length                                            |
| mprotect        | Memory Management | Sets memory protection attributes.                        | void *addr, size_t len, int prot                                     |
| socket          | Networking        | Creates a socket for network communication.               | int domain, int type, int protocol                                   |
| bind            | Networking        | Binds a socket to an address/port.                        | int sockfd, const struct sockaddr *addr, socklen_t addrlen           |
| listen          | Networking        | Marks a socket as passive for accepting connections.      | int sockfd, int backlog                                              |
| accept          | Networking        | Accepts a new connection on a socket.                     | int sockfd, struct sockaddr *addr, socklen_t *addrlen                |
| connect         | Networking        | Establishes a connection to a remote host.                | int sockfd, const struct sockaddr *addr, socklen_t addrlen           |
| send            | Networking        | Sends data over a socket.                                 | int sockfd, const void *buf, size_t len, int flags                   |
| recv            | Networking        | Receives data from a socket.                              | int sockfd, void *buf, size_t len, int flags                         |
| gettimeofday    | Time Management   | Retrieves the current time.                               | struct timeval *tv, struct timezone *tz                              |
| nanosleep       | Time Management   | Suspends execution for a specified time.                  | const struct timespec *req, struct timespec *rem                     |
| settimeofday    | Time Management   | Sets the system time.                                     | const struct timeval *tv, const struct timezone *tz                  |
| clone           | Process Control   | Creates a new process/thread.                             | int flags, void *child_stack                                         |
| sched_yield     | Process Control   | Yields CPU time to another process.                       | void                                                                 |
| getuid          | User Management   | Returns the user ID of the calling process.               | void                                                                 |
| setuid          | User Management   | Sets the user ID of the calling process.                  | uid_t uid                                                            |
| getgid          | User Management   | Returns the group ID of the calling process.              | void                                                                 |
| setgid          | User Management   | Sets the group ID of the calling process.                 | gid_t gid                                                            |

# Links
* [Github Lecture](https://linux-kernel-labs.github.io/refs/heads/master/lectures/syscalls.html)